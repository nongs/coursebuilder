...
|------|-------------------|
| `unknown` | 타입 미선택 안내 |
| `text` | 제목, 본문(content) — TipTap HTML, 저장·로드 시 DOMPurify 적용 |
| `video` | `videoUrl`, YouTube URL 패턴이면 미리보기 |
| `quiz` | 단답형/서술형/객관식. 서술형 **평가기준(rubric)** 은 일반 텍스트 입력(`textarea`). 객관식은 보기 추가·삭제(최소 1개), 정답 라디오 선택 |

공통: 추가/삭제. 본문(content)은 리치 텍스트(TipTap)로 편집하고, 에디터는 `common/textEditor` 모듈에서 lazy 로드됨. 이미지는 **http(s) 공개 URL**만 저장(모달에서 입력).
